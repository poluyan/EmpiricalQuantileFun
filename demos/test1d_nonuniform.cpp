...
int main()
{
	...
	typedef mveqf::Trie<mveqf::NodeCount<int>,int> trie_type;
	std::shared_ptr<trie_type> sample = std::make_shared<trie_type>();
	sample->set_dimension(dimension);
	sample->insert(std::vector<int> {1}, 3);
	sample->insert(std::vector<int> {2}, 1);
	sample->insert(std::vector<int> {3}, 2);
	sample->insert(std::vector<int> {7}, 3);
	sample->insert(std::vector<int> {8}, 4);
	sample->insert(std::vector<int> {9}, 2);
	sample->insert(std::vector<int> {10}, 1);
