...
//			size_t get_node_count() const;
		...
		protected:
			...
			void is_all_empty(TNode *p) const;
			void delete_last(int dim);
			void destroy_recursive(TNode *p);
			size_t dimension;
		};
		
		template <typename TNode, typename TIndex>
		void TrieBased<TNode,TIndex>::destroy_recursive(TNode *p)
		{
			if(!p->children.empty())
			{
				for(auto &i : p->children)
		...
		template <typename TNode, typename TIndex>
		TrieBased<TNode,TIndex>::TrieBased(size_t dim)
		{
			root = new TNode();
			dimension = dim;
		}
