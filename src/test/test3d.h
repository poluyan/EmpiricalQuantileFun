...
void test_3d_kquantile2()
{
    ...
    {
        std::vector<T> shift = {5, 5, 5};
        T tt = lognormal(generator);
        while(tt > 25)
            tt = lognormal(generator);
        std::vector<T> t = {-5.0 + tt, norm(generator), norm(generator)};
        for(size_t j = 0; j != t.size(); j++)
            t[j] += shift[j];
        T x = t[0];
        T y = t[1];
        T tau = 3.1415926/4;//acos(-1.0)/2.0;
        t[0] = x*std::cos(tau) - y*std::sin(tau) + 12;
        t[1] = y*std::cos(tau) + x*std::sin(tau);


        x = t[0];
        y = t[2];
        tau = 3.1415926/4;//acos(-1.0)/2.0;
    ...
    data_io::write_default2d("maps/Trie/3d2/sampled3d_init.dat", output, 5);
