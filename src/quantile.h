...
template <typename T, typename U>
void Quantile<T, U>::transform(const std::vector<U>& in01, std::vector<U>& out) const
{
...
template <typename T, typename U>
std::pair<size_t, U> Quantile<T, U>::ecdf1d_pair_fromgrid_trie(const std::vector<std::pair<T, T>> &row, size_t sample_size, size_t ind, float val01) const
{
